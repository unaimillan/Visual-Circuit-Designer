...
LoginHandler::LoginHandler(DBConnector& db, TokenManager& tokenManager): m_db(db), m_tokenManager(tokenManager) {}
...
void LoginHandler::handleRequest(HTTPServerRequest& request, HTTPServerResponse& response) {
    ...
    logger.information("POST /api/login from %s", request.clientAddress().toString());
