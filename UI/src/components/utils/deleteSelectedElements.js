export function deleteSelectedElements(nodes, edges, clipboard) {
  ...
  const adjacentEdgeIds = new Set(edges.filter(
    (edge) => selectedNodeIds.has(edge.source) ||
  ...
  const newEdges = edges.filter((edge) => !selectedEdgeIds.has(edge.id) && !adjacentEdgeIds.has(edge.id));
